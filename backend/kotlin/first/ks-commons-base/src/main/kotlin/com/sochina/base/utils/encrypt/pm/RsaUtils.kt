...
object RSAUtils {
     ...
     */
    @Throws(Exception::class)
    private fun decryptByPrivateKey(encryptedData: ByteArray, privateKey: String): ByteArray {
        val keyBytes: ByteArray? = Base64Utils.decode(privateKey)
        val pkcs8KeySpec = PKCS8EncodedKeySpec(keyBytes)
        val keyFactory = KeyFactory.getInstance(KEY_ALGORITHM)
     ...
     */
    @Throws(Exception::class)
    private fun decryptByPublicKey(encryptedData: ByteArray, publicKey: String): ByteArray {
        val keyBytes: ByteArray? = Base64Utils.decode(publicKey)
     ...
     */
    @Throws(Exception::class)
    private fun encryptByPrivateKey(data: ByteArray, privateKey: String): ByteArray {
        val keyBytes: ByteArray? = Base64Utils.decode(privateKey)
        val pkcs8KeySpec = PKCS8EncodedKeySpec(keyBytes)
        val keyFactory = KeyFactory.getInstance(KEY_ALGORITHM)
        val privateK: Key = keyFactory.generatePrivate(pkcs8KeySpec)
    ...
    fun encryptedDataOnServer(data: String?, publicKey: String): String? {
        return if (data.isNullOrBlank()) {
            ...
            Base64Utils.encode(encryptByPublicKey(data.toByteArray(), publicKey))
        }
    }
    ...
    fun decryptDataOnServer(data: String?, privateKey: String): String? {
        return if (data.isNullOrBlank()) {
            ...
            String(decryptByPrivateKey(Base64Utils.decode(data)!!, privateKey), StandardCharsets.UTF_8)
        }
    }
}
