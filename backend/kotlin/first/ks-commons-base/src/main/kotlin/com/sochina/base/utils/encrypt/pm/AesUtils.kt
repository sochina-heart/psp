...
object AESUtils {
     ...
     * @return
     */
    fun encryptEcb(content: String?, aesKey: String): String? {
     ...
     * @return
     */
    fun decryptEcb(content: String?, aesKey: String): String? {
        if (content.isNullOrBlank()) {
     ...
     * @return
     */
    fun encryptCbc(content: String?, aesKey: String): String? {
        if (content.isNullOrBlank()) {
     ...
     * @return
     */
    fun decryptCbc(content: String?, aesKey: String): String? {
        if (content.isNullOrBlank()) {
            LOGGER.info("AES_CBC decrypt: the content is null!")
            return null
        }
        // 判断秘钥是否为16位
        if (aesKey.isNullOrBlank() && aesKey.length == 16) {
            try {
                // 对密码进行编码
                val bytes = aesKey.toByteArray(charset(ENCODING))
                // 设置解密算法，生成秘钥
